...
import { BtStrategyModel } from '../data-models/btStrategy.js';
...
export type BtStrategyDao = {
  composeWith: <R>(fn: (internal: { mongooseModel: BtStrategyMongooseModel }) => R) => R;
};

export function buildBtStrategyDao(
  client: Mongoose,
): ioe.IOEither<BtStrategyDaoError<'BuildDaoFailed'>, BtStrategyDao> {
  return pipe(
...
function createMongooseModel(
  client: Mongoose,
): ioe.IOEither<BtStrategyDaoError<'BuildDaoFailed'>, BtStrategyMongooseModel> {
