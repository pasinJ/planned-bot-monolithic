...
import { randomTimeframe, randomTimeframeExclude } from '#test-utils/domain.js';
...
describe('Get list of months', () => {
  describe('WHEN get list of months', () => {
    it('THEN it should return a list of months in the given range', () => {
...
describe('Get klines from monthly files', () => {
  ...
  describe('WHEN start getting klines from monthly files', () => {
    it('THEN it should download file for each month in the date range', async () => {
      const deps = mockDeps();
  ...
  describe('WHEN less than last 2 downloading at the end of the date range fail with Not Found error', () => {
    describe('GIVEN timeframe is 1s, 1m, or 3m', () => {
    ...
    describe('GIVEN timeframe is not 1s, 1m, or 3m', () => {
      it('THEN it should call API fallback method with date range from start of the failed downloading to the end of date range parameter', async () => {
        const deps = mockDeps({
          httpClient: {
            downloadFile: jest
              .fn()
    ...
    describe('WHEN some of fallback method fails', () => {
      it('THEN it should return Left of error', async () => {
        const deps = mockDeps({
          ...
          bnbService: {
            getKlinesFromDailyFiles: () => te.left(readCsvError),
