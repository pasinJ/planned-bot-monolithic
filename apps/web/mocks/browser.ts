...
export const worker = setupWorker(
  rest.get(getPortfoliosUrl, (_, res, ctx) => res(ctx.status(200), ctx.json([]))),
  rest.post(createPortfoliosUrl, (_, res, ctx) => res(ctx.status(201), ctx.json(mockCreatePortfolioResp))),
);
